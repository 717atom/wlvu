...
], W = (e, t) => `${e.toString().padStart(2, "0")}:${t.toString().padStart(2, "0")}`;
async function G() {
	try {
		let e = await (await caches.open("notification-settings")).match("settings");
		if (e) return await e.json();
	} catch {
...
function K(e) {
	let { ifWindowStart: t, customMealTimes: n } = e;
	return n && n.length === H.length ? H.map((e, t) => {
		let r = n.find((t) => t.id === e.id) || n[t];
		return {
			...e,
			time: W(r.hour, r.minute),
			hour: r.hour,
			minute: r.minute
		};
	}) : H.map((e, n) => {
		let r = U[n], i = (t + r.hours) % 24, a = r.minutes;
		return {
			...e,
			time: W(i, a),
			hour: i,
...
async function J() {
	...
	let t = /* @__PURE__ */ new Date(), n = W(t.getHours(), t.getMinutes()), r = t.toDateString(), i = K(e);
	for (let t of i) {
		if (!e.enabledMeals.includes(t.id)) continue;
		let i = `${r}-${t.id}`;
		t.time === n && !q.has(i) && (q.add(i), await self.registration.showNotification(`${t.time} - ${t.label}`, {
			body: t.description,
			icon: "/favicon.svg",
			badge: "/favicon.svg",
			tag: `meal-${t.id}`,
	...
	for (let e of q) e.startsWith(r) || q.delete(e);
}
self.addEventListener("message", async (e) => {
	e.data && e.data.type === "SYNC_NOTIFICATION_SETTINGS" && (await (await caches.open("notification-settings")).put("settings", new Response(JSON.stringify(e.data.settings))), J()), e.data && e.data.type === "CHECK_NOTIFICATIONS" && J();
}), self.addEventListener("install", (e) => {
	e.waitUntil((async () => {
		await self.skipWaiting();
		try {
			let e = self.registration;
			e.periodicSync && (await e.periodicSync.register("check-meal-notifications", { minInterval: 6e4 }), console.log("SW: Registered periodic sync"));
		} catch {
	...
	e.waitUntil((async () => {
		...
		await Promise.all(e.filter((e) => e !== "notification-settings").map((e) => (console.log("SW: Deleting old cache:", e), caches.delete(e)))), await J(), await self.clients.claim(), setInterval(J, 3e4);
	})());
}), self.addEventListener("sync", (e) => {
	e.tag === "check-meal-notifications" && e.waitUntil(J());
}), self.addEventListener("notificationclick", (e) => {
	e.notification.close(), e.waitUntil(self.clients.matchAll({
		type: "window",
		includeUncontrolled: !0
	}).then((e) => {
		for (let t of e) if ("focus" in t) return t.focus();
